...
internal fun View.screenshot(
    ...
    try {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            PixelCopy.request(
                ...
                bitmap,
                {
                    when (it) {
                        PixelCopy.SUCCESS -> {
                            bitmapCallback.invoke(FeinnScreenshotResult.Success(bitmap.asImageBitmap()))
                        }
                        PixelCopy.ERROR_DESTINATION_INVALID -> {
                            bitmapCallback.invoke(
                                FeinnScreenshotResult.Error(
                                    Exception(
        ...
        bitmapCallback.invoke(FeinnScreenshotResult.Error(e))
    }
}